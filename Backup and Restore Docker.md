Since you are running Wazuh inside Docker on Windows, the configuration files, agent keys, and security logs do not live inside the actual containers—they live in **Docker Named Volumes**.

To back up and restore Wazuh, you need to extract the data from these volumes into a compressed file on your Windows host machine, and reverse the process to restore it.
